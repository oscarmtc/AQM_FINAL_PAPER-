...
# ... we finalize the procedure by eliminating the created reference objects. The purpose of this step is to "clean" the working environmebt by getting rid of objects that will, not be used anymore ...
...
This initial part of the analysis consists of the following sub-section: 
     1.1: We initiate by defining the dependent and independent variables (both models). 
     1.2: We then optimize the log-likelihood functions given the inputs defined in step 1.1 (both models).
     1.3: We display the results in a nice-formatted table (both models).
     1.4: Using the results from part 1.2, we estimate Quantities of Interest (QoI's):
        1.4.1: Definition of the Simulation Function 
        1.4.2: Predicted Probabilities
        1.4.3: First Differences 
    
```{r Part 1.1.: Definition of Dependent and Independent Variables}
...
```{r Part 1.2.: Optimization of Logit-Log-Likelihood Functions}
...
```{r Part 1.3.: Display and Visualization of Regression Results}
...
```{r Part 1.4.1: Definition of the Simlation Function}
...
```{r Part 1.4.2: Definition of the Key Quantities and Distributions for Simlation}
...
# ... and then define the corresponding variance-covariance matrices for models 1 and 2 (respectively) ...
varcov_m1E1 <- solve(-res2_m1E1$hessian)
varcov_m2E1 <- solve(-res2_m2E1$hessian)

# ... having conducted the previous steps, we set up the sampling distribution for each of the two models ...
S_m1E1 <- mvrnorm(n_simulations, res2_m1E1$par, varcov_m1E1) # ... for model 1 ...
S_m2E1 <- mvrnorm(n_simulations, res2_m2E1$par, varcov_m2E1) # ... for model 2 ...

# ... we finalize by inspecting the dimensions of the objects "S_m1E1" and "S_m1E1" ...
dim(S_m1E1) # ... for model 1 ...
dim(S_m2E1) # ... for model 2 ...

```
```{r Part 1.4.3: Definition of Scenarios}
...
```{r Part 1.4.4: Estimation of Predicted Probabilities Using the Average Value Approach}
...
dev.copy2pdf(file = ("Predicted_Probabilities.pdf"), 
             family = "Times", 
             width = 12, 
             height=6)
#_____________________________________________________________________________#
...
```{r Part 1.4.4: Estimation of First Differences}
...
#_____________________________________________________________________________#
# ... the following command lines save the generated plots into a single PDF-file ...
dev.copy2pdf(file = ("First_Differences_AVA.pdf"), 
             family = "Times", 
             width = 12, 
             height=6)
#_____________________________________________________________________________#
...
```{r Part 1.4.4: Estimation of Predicted Probabilities Using the Observed Value Approach}
