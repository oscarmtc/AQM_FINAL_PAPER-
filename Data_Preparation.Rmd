...
STATA_REF2 <- read_stata("STATA_Reference2.dta") 
 # ... the following commands generate a series of data frames ("reference4a_STATA" -> "reference4h_STATA") containing the lagged dependent variables from the original STATA output ... 
...
# ... we finalize the procedure by eliminating thecreated reference objects ...
rm(reference4a_STATA, 
   reference4a_STATA, 
   reference4b_STATA, 
   reference4c_STATA, 
   reference4d_STATA, 
...
# ... in order to ensure that the change variables are exactly the same as in the original STATA-output, we append the corresponding columns from the original STATA output ...
 # ... the following command generates a data frame containing the columns with the change dependent variables from the original STATA output ... 
reference5_STATA <- STATA_REF2[, c("lsvim_vox", "lsauthoritarian", "lsideol", "lsnativism", "lsorgterr", "lspop6amz", "lsmsexism", "lsswim_msex")]
# ... we then append the columns from the original STATA-output into our main data frame ...
data_panel$coh3 <- reference3_STATA$coh3
mean(data_panel$coh3, na.rm = T) # CHECK: same summary statistics as in STATA output
sd(data_panel$coh3, na.rm = T) # CHECK: same summary statistics as in STATA output
...
mean(data_panel$lsvim_vox, na.rm = T)
...
mean(data_panel$lsmsexism, na.rm = T)
mean(data_panel$lsswim_msex, na.rm = T)
