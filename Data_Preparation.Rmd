...
data_panel$abstain_vote <- ifelse(is.na(data_panel$rv16all), 1, 0)
...
```
##Analysis Part 1 : Replication and Reproduction of the Paper's Original Results (Tables 1 & 2)

This initial part of the analysis has the purpose of replicating Tables 1 and 2 of the original paper. Even though they will not be part of our contribution, presenting these table will allow us to assess the following aspects: 
...
# ... we then proceed to remove the observations with missing values for the specified variables, storing the remaining observations in a data frame called "rep_m1T1" ...
for (var in variables_m1_T1) {
  rep_T1 <- data_panel[complete.cases(data_panel[, var]), ]
}

# ... we first create a subset of the main data frame containing only observations for which the value of "year" is 2019 ...
rep_m1T1 <- subset(rep_T1, year == 2019) 

# ... we first define the dependent variable (intended vote for Vox at time "t" - "vim_vox") ...
Y_rm1_T1 <- rep_m1T1$vim_vox
...
independent_variables_m1_T1  <- c("female",
                                  "age",
                                  "edu3_2",
                                  "edu3_3",
                                  "dhincome_all",
...
independent_variables_m1 <- rep_m1T1[, independent_variables_m1_T1]
...
# ... based on the subset for the replication of Table 1 generated above ("rep_T1"), we create a subset containing only observations for which the value of "year" is 2020 ...
rep_m2T1 <- subset(rep_T1, year == 2020) 

# ... we first define the dependent variable (intended vote for Vox at time "t" - "vim_vox") ...
Y_rm2_T1 <- rep_m2T1$vim_vox
Y_rm2_T1 <- as.vector(Y_rm2_T1) # we store the y-vector as a numeric vector

# ... and proceed to define the set of independent variables
independent_variables_m2_T1  <- c("female",
                                  "age",
                                  "edu3_2",
                                  "edu3_3",
                                  "dhincome_all",
...
independent_variables_m2 <- rep_m2T1[, independent_variables_m2_T1]
...
startvalues2__rm1 <- round(res1_logit_rm1$par,10)

res2_logit_rm1 <- optim(par= startvalues2__rm1,
             fn = LL_logit1,
             y = Y_rm1,
             X = X_rm1,
             control=list(parscale=abs(res1_logit_rm1$par), fnscale = -1),
             hessian = TRUE,
             method = "BFGS"
             )

# ... we return the point estimates and store these in an object called ...
point_estimates_rm1 <- res2_logit_rm1$par[1:22]

# ... We proceed with the estimation of the standard errors ...
SE_rm1 <- sqrt(diag(solve(-res2_logit_rm1$hessian)))
...
P_Values_rm1 <- 2 * (1 - pnorm(abs(point_estimates_rm1 / SE_rm1)))

# ... we additionally extract the number of observations ...
N_rm1 <- length(res2_logit_rm1$Y_rm1)
...
##Analysis Part 2 : Replication and Reproduction of the Paper's Original Results

This initial section of the statistical analysis has the goal of reproducing the original model specification presented in the paper. For this part, no modifications (either in the operationalization of key concepts or in the exact specification of the models are conducted). 
This initial section consists of three main sub-sections (I., II., and III.): 
      I. Develops the log-likelihood function and corresponding optimization to find the relevant parameters ... 
          I.1 As in the original paper, we develop a series of logistic regression models.
          I.2 We additionally estimate the same models using probit regression models. 
      II. Relies on the build-in functions and commands to ensure the correctness of the initial point estimates (coefficients) and of the uncertainty around them. 
      III. Presents the results of the estimation in a suitable and well-formatted table. 

``` {r Definition of Dependent and Independent Variables}

# (A) We then proceed to define the set of dependent and independent variables; we apply a separate procedure for each of the four models presented in the original paper ...

# .............................................................................#
# ............................... (A.1) - MODEL 1 ..............................
# .............................................................................#

# ... we define a vector containing the names of the independent variables (as stored in the main data frame) that will be used for the estimation of the model.
independent_variables <- c("female",
                           "age",
                           "edu3_2",
                           "edu3_3",
                           "dhincome_all",
...
# we first create a subset of the main data frame containing only observations for which the value of "year" is 2019. 
subset_data <- data_panel

rep_model1 <- na.omit(subset_data[, c(independent_variables, "vim_vox")])

rep_model1 <- subset(rep_model1, year == 2019) 


# ... and proceed to remove missing values for the specified independent variables
rep_model1 <- na.omit(rep_model1[, c(independent_variables, "vim_vox")])

# ... we first define the dependent variable (intended vote for Vox at time "t" - vim_vox)
Y_rm1 <- rep_model1$vim_vox
Y_rm1 <- as.numeric(Y_rm1) # we store the y-vector as a numeric vector

# ... and proceed to define the set of independent variables
independent_variables_m1 <- rep_model1[, independent_variables]
X_rm1 <- cbind(1, independent_variables_m1)
X_rm1 <- as.matrix(sapply(X_rm1, as.numeric)) # we make sure that the matrix X is indeed saved as a matrix
dim(X_rm1)
...
# ... we initiate by defining the starting values (one for each covariate plus one for the intercept) ...
startvalues1__rm1 <- rep(0,22)

## ... and implement an initial optimization ...
res1_logit_rm1 <- optim(par = startvalues1__rm1,
             fn = LL_logit1,
             y = Y_rm1,
             X = X_rm1,
             control = list(fnscale = -1),
...
results_rm1 <- data.frame(
  ...
  Point_Estimates = point_estimates_rm1,
  Standard_Errors = SE_rm1,
  P_Values = P_Values_rm1,
  Observations = N_rm1 )

# Print formatted table
kable(results_rm1, align = "c", digits = 7, caption = "Results Replication Model 1",
...
```
"
##Analysis Part 2 : Expansion - Identification of Gender-induced Differences 
