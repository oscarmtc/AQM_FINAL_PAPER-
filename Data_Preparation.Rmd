...
``` {r Generation of cohort variable}
...
```{r Generation of variables education and partner}
...
```{r Generation of variable for income}

# (11): Generation of the variable for respondent's income 

data_panel$hincome_all <- data_panel$hhincome
  
data_panel$hincome_all <- ifelse(data_panel$hincome_all==99, NA, data_panel$hincome_all)

mean(data_panel$hincome_all,  na.rm = TRUE) # everything is cooool
...
data_panel$hincome_17 <- ifelse(data_panel$year == 2017, data_panel$hincome_all, NA)
data_panel$hincome_18<- ifelse(data_panel$year == 2018, data_panel$hincome_all, NA)
data_panel$hincome_19 <- ifelse(data_panel$year == 2019, data_panel$hincome_all, NA)
data_panel$hincome_20 <- ifelse(data_panel$year == 2020, data_panel$hincome_all, NA)
...
mean(data_panel$hincome17all, na.rm = TRUE) #work for 17 (mean = 6.31945), 18 (mean = 6.378386), 19 (mean = 6.484415), 20 ( mean = 6.48741). i compared with STATA_DATA
...
mean(data_panel$hincome_all, na.rm=TRUE) # similar to mean in Stata_Data (6.417858)

# Recode hincome_all to x3hincall
data_panel$x3hincall <- cut(data_panel$hincome_all, breaks = c(0, 5, 8, 12), labels = FALSE, include.lowest = TRUE)

# Generate dhincome_all
data_panel$dhincome_all <- (data_panel$hincome_all - 1) / 11

```

```{r interest in politics, ideology, authoritarianism, nativism, populism and territory}
...
``` {r Generation of Variable for sexism}
...
```{r Generation of Variables for Vote Intention}
...
``` {r Generation of variables for turnout and vote choices}
...
``` {r Visualization of All Variables' Distribuitions}
