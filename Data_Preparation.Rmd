...
This initial part has the following sub-sections: 
                  ...
                  Part 1.5: Comparison of the results with the in-built command for logistic regression estimation
...
# ... we define a vector containing the names of all the independent variables (as stored in the main data frame) that will be used for the estimation of the model ...

variables_re_T2  <- c("female",                 # IV  ...  NOTE: IV > Independet Variable  
                      "age",                    # IV
                      "edu3_2",                 # IV
                      "edu3_3",                 # IV
                      "dhincome_all",           # IV
...
data_panel2 <- na.omit(data_panel[, c(variables_re_T2, "vim_vox")])
...
out2 <- cbind(point_estimates_rm2_T1, SE_rm2_T1)
colnames(out2) <- c('Logit Standard Errors', 'Scobit Standard Errors')
rownames(out2) <- c("Intercept",
                   "Commodity Dependence",
                   "Squared Commodity Dependence",
                   "Male Secondary Schooling",
                   "GDP Growth",
...
out3 <- cbind(P_Values_logit, P_Values_scobit)
colnames(out3) <- c('Logit p-Values', 'Scobit p-Values')
rownames(out3) <- c("Intercept",
                   "Commodity Dependence",
                   "Squared Commodity Dependence",
                   "Male Secondary Schooling",
                   "GDP Growth",
...
unified_table <- cbind(out1, out2, out3)

kable(unified_table,  digits = 6, align = "ccrl",
      booktabs = T,
      caption = "Comparison of the Logit and Scobit Results for the Estimated Model") %>%
