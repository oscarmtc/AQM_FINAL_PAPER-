...
data_panel$hincome_all <- replace(data_panel$hhincome, data_panel$hhincome == 99, NA)
...
#firstly we need to create separate variables for each year
    ...
    data_panel$hincome_17 <- ifelse(data_panel$year == 2017, data_panel$hhincome, NA_real_)
     data_panel$hincome_18<- ifelse(data_panel$year == 2018, data_panel$hhincome, NA_real_)
      data_panel$hincome_19 <- ifelse(data_panel$year == 2019, data_panel$hhincome, NA_real_)
       data_panel$hincome_20 <- ifelse(data_panel$year == 2020, data_panel$hhincome, NA_real_)
...
data_panel <- data_panel %>%
  group_by(idcode) %>%
  mutate(
    hincome17all_tmp = ifelse(is.na(hincome_17) & !is.na(dplyr::lead(hincome_17)), dplyr::lead(hincome_17), hincome_17),
    hincome18all_tmp = ifelse(is.na(hincome_18) & !is.na(dplyr::lead(hincome_18)), dplyr::lead(hincome_18), hincome_18),
    hincome19all_tmp = ifelse(is.na(hincome_19) & !is.na(dplyr::lead(hincome_19)), dplyr::lead(hincome_19), hincome_19),
    hincome20all_tmp = ifelse(is.na(hincome_20) & !is.na(dplyr::lead(hincome_20)), dplyr::lead(hincome_20), hincome_20)
  ) %>%
  ungroup()
    ...
    hincome_all = ifelse(year == 2017, hincome17all,
                        ifelse(year == 2018, hincome18all,
                               ifelse(year == 2019, hincome19all,
                                      ifelse(year == 2020, hincome20all, NA))))
  ) %>%
  select(-matches("^hincome(17|18|19|20)$"))  

# Recode hincome_all to x3hincall
data_panel$x3hincall <- cut(data_panel$hincome_all, breaks = c(0, 5, 8, 12), labels = c("Low", "Mid", "High"), include.lowest = TRUE)

# Generate dhincome_all
data$dhincome_all <- (data$hincome_all - 1) / 11
#
