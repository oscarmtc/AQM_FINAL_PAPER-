...
data_panel <- data_panel %>%
  group_by(idcode) %>%
  mutate(
    hincome17all = max(hincome_17, na.rm = TRUE),
    hincome18all = max(hincome_18, na.rm = TRUE),
    hincome19all = max(hincome_19, na.rm = TRUE),
    hincome20all = max(hincome_20, na.rm = TRUE)
  ) %>%
  ungroup()
...
data_panel <- data_panel %>%
  ...
  select(-hincome17all, -hincome18all, hincome19all, hincome20all)  
