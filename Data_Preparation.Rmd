...
##Analysis Part 1 : Replication and Reproduction of the Paper's Original Results
...
``` {r Definition of Logit-Log-Likelihood Function}

LL_logit1 <- function(theta, y, X) {
...
LL_logit1_penalized <- function(theta, y, X, lambda) {
    beta <- theta[1:ncol(X)]
    # definition of linear predictor mu ...
    mu <- X %*% beta
    # definition of logit-link function ...
...
rep_model1 <- subset(data_panel, year == 2019) # we first create a subset of the main data frame containing only observations for which the value of "year" is 2019. 
for (var in independent_variables) {
  rep_model1 <- rep_model1[complete.cases(rep_model1[, var]), ]
}
...
cv_model <- cv.glmnet(X_rm1, Y_rm1, alpha = 0, nfolds = 10)

# Plot the cross-validation curve
plot(cv_model)

# Get the lambda value with the lowest cross-validated error
...
kable(results_rm1, align = "c", digits = 7, caption = "Results Replication Model 1",
      col.names = c("Variables", "Point Estimates", "Standard Errors", "P-Values"),
